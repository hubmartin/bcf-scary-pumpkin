...
#define PIR_PUB_MIN_INTERVAL  (5 * 1000)
...
void pir_event_handler(bc_module_pir_t *self, bc_module_pir_event_t event, void *event_param)
{
    ...
    if (event == BC_MODULE_PIR_EVENT_MOTION)
    {
        ...
        if (pir_next_pub < bc_scheduler_get_spin_tick())
        {
            ...
            pir_next_pub = bc_scheduler_get_spin_tick() + PIR_PUB_MIN_INTERVAL;
        }
    }
}
...
void application_init(void)
{
    ...
    bc_servo_init(&servo, BC_PWM_P14);
    bc_servo_set_angle(&servo, 90);
